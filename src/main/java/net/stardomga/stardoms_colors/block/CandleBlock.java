...
public class CandleBlock extends net.minecraft.block.CandleBlock implements BlockEntityProvider {
    ...
    */
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (stack.getItem() instanceof DyeItem dyeItem) {
            if (world.getBlockEntity(pos) instanceof CandleBlockEntity colorBlockEntity) {
    ...
    protected ItemStack getPickStack(WorldView world, BlockPos pos, BlockState state, boolean includeData) {
        if (world.getBlockEntity(pos) instanceof CandleBlockEntity be) {
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (world.getBlockEntity(pos) instanceof CandleBlockEntity coloredBE) {
            // Get the number of candles from the block state
            int candleCount = state.get(CANDLES);

            // Create the item stack with the correct color component and count
            ItemStack coloredCandles = coloredBE.getColoredDrop();
            coloredCandles.setCount(candleCount); // Set stack size to match candle count
